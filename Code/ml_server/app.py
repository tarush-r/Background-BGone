...
def get_user_images():
    ...
    print(user_id)
    originalImagesBlob = bucket.list_blobs(prefix=f"users/{user_id}/original/")
    segmentedImagesBlob = bucket.list_blobs(prefix=f"users/{user_id}/segmented/")
    print(originalImagesBlob)
    hMap = {}
    originalList = []
    segmentedList = []
    print('hello')
    if not segmentedImagesBlob:
        return jsonify({"message": "No images found for user ID"}), 404

    # for i, blob in enumerate(originalImagesBlob):
    for blob in originalImagesBlob:
        
        url = blob.generate_signed_url(expiration=expiration_time)
        # url = blob.public_url
        originalList.append(url)
    print('hello')
    
    for i, blob in enumerate(segmentedImagesBlob):
        url = blob.generate_signed_url(expiration=expiration_time)
        # url = blob.public_url
        segmentedList.append(url)
    print('hello')
    
    print(originalList)
    hMap['segmented'] = segmentedList
    hMap['original'] = originalList
    return jsonify(hMap)
